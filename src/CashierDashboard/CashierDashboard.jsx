              ...
              <div className="col-12 col-lg-8 mb-4">
                <div className="d-flex flex-column">
                  <div className="d-flex flex-wrap gap-3 mb-4">
                    {[{ name: "All Menu" }, { name: "Foods", icon: "/assets/reserve.svg" }, { name: "Beverages", icon: "/assets/coffee.svg" }, { name: "Dessert", icon: "/assets/cake.svg" }]
                      .map((category) => (
                        <button
                          key={category.name}
                          className={`btn d-flex align-items-center justify-content-center ${activeButton === category.name ? "btn-primary text-white" : "btn-outline-primary"}`}
                          style={{ padding: "5px 10px", minWidth: "150px", height: "55px" }}
                          onClick={() => handleButtonClick(category.name)}
                        >
                          {category.icon && <img src={category.icon} alt={`${category.name} Icon`} style={{ width: "20px", height: "20px", marginRight: "8px" }} />}
                          {category.name}
                        </button>
                    ))}
                  </div>
                  ...
                  <div className="position-relative">
                    <div className="mb-2">
                      <h2>List Menu</h2>
                      <span className="text-muted">Total: {filteredMenu.length} Menu</span>
                    </div>
                    <div className="scroll-container" style={{ maxHeight: "600px", overflowY: "auto" }}>
                        ...
                        {filteredMenu.map((menu) => (
                          ...
                          >
                            <div className="card h-100">
                              <span className="badge bg-primary" style={{ fontSize: "0.7rem", padding: "3px 8px", borderRadius: "20px", position: "absolute", top: "5px", right: "5px", zIndex: "10" }}>
                                {menu.category}
                              </span>
                              <img src={menu.image} className="card-img-top" alt={menu.name} style={{ height: "100px", objectFit: "cover" }} />
                              <div className="card-body">
                                <h6 className="card-title" style={{ fontSize: "0.9rem" }}>{menu.name}</h6>
                                <p className="card-text text-muted" style={{ fontSize: "0.8rem" }}>{menu.description}</p>
                                <p className="card-text" style={{ fontSize: "0.9rem" }}>
                                  <span style={{ color: "blue", fontWeight: "bold" }} >
                                    {formatPriceToIDR(menu.price)}
                                  </span> <span style={{ color: "black" }}>/portion</span>
                                </p>
                              </div>
                            </div>
                          </div>
